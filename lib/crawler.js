...
(function () {
  ...
  function getMenu ( url) {
  		request.get({
  			...
  			if (err) {
  				console.log(err + ', request again after 5 seconds!');
  				setTimeout(function(){
  					return getMenu(url);
  				}, 5000);
  			} else {
            ...
            console.log('Get ' + mainUrlLists.length + ' urls!');
  				});
  ...
  function getArticle (url) {
    request.get({
      ...
      if (err) {
        console.log(err + 'request the article again after 5 seconds');
        setTimeout(function(){
					return getArticle(url);
				}, 5000);
      } else {
        var $ = cheerio.load(body.toString()),
				    article = {
    					...
    					comment: getPushContents ($),
    					content: getArticleMainContent ($)
  				    };
        resultContent.push(article);
        console.log('Get' + article.title + 'in memory!');
        return requestArticle();
      }
    });
  }
  ...
  function getPushContents ($) {
  	var pushContents = [];
  	$('.push').each(function () {
  		var $me = $(this),
  		    pushContent = {
      			tag: $me.find('.push-tag').text(),
      			user: $me.find('.push-userid').text(),
      			content: $me.find('.push-content').text(),
      			ipdatetime: $me.find('.push-ipdatetime').text()
      		};
  		pushContents.push(pushContent);
  	});
  	return pushContents;
  }
  /**
